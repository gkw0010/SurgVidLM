...
def inference_batch(result_folder, model_path, test_data_path):
    ...
    result_path = f"{result_folder}/full_result.json"  #check
    ...
    print(len(test)) 
    print("start-end:",test[0]['total_id'],test[-1]['total_id'])
    ...
    tested_ids=[item['total_id'] for item in results]
    print(f"already tested:{len(tested_ids)}")

    for i,item in enumerate(test):  #check
        # if i==10:
        #     break
        if test[i]['total_id'] not in tested_ids: #check 
            print(test[i]['total_id'])
            ...
            if i%1 ==0 or i==len(test)-1:
                ...
                print(result_json['total_id'],result_json['video'],result_json['tag'])
                # print(result_json)
...
if __name__ == "__main__":
    ...
    # inference_batch(result_folder, model_path, test_data_path)
