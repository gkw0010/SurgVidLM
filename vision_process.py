...
def _read_video_torchvision(
    ...
    print(f"torchvision:  {video_path=}, {total_frames=}, {video_fps=}, time={time.time() - st:.3f}s")
    nframes = smart_nframes(ele, total_frames=total_frames, video_fps=video_fps)
    print(f'number of sample frames: {nframes}')
...
def _read_video_decord(
    ele: dict,
) -> (torch.Tensor, torch.Tensor | None, float):
...
def get_video_reader_backend() -> str:
    if FORCE_QWENVL_VIDEO_READER is not None:
        ...
        assert False, "please download decord first by 'pip install decord'"
...
if __name__ == "__main__":
    ...
    if image_inputs is not None:
        print(f"Image shape: {image_inputs[0].shape}")
        
    if video_inputs is not None:
        print(f"Video shape: {video_inputs[0].shape}, Clip shape: {clip_inputs[0].shape}")
